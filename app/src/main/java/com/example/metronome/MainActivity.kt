...
data class Song(
    ...
    val id: Int = 0,
    val userId: String, // Dodane pole na ID użytkownika
    val title: String,
    val artist: String,
...
class SongViewModel(private val repository: SongRepository) : ViewModel() {
    ...
    suspend fun loadSongs() {
        currentUserId?.let { userId ->
            ...
            val userSongs = repository.getAllSongsForUser(userId)
            if (userSongs.isEmpty()) {
                // Jeśli użytkownik nie ma żadnych piosenek, dodaj przykładowe
...
//    }
                        ...
                        if (showSplash) {
                            ...
                            if (user != null) {
                                MetronomeApp(
                                    onSignOut = {
                                        auth.signOut()
                                        // Odświeżenie interfejsu po wylogowaniu
                                        setContent { MetronomeApp(onSignOut = {}) }
                                    }
                                )
                            } else {
                                SignInScreen(
                                    onSignIn = {
                                        // Odświeżenie interfejsu po zalogowaniu
                                        setContent { MetronomeApp(onSignOut = {}) }
                                    }
                                )
                            }
    ...
    fun SettingsScreen(
        ...
        if (!isInPreview) {
            ...
            val viewModel: SongViewModel = viewModel(factory = SongViewModel.SongViewModelFactory(
                repository
            )
            )
            ...
            // Użyj LaunchedEffect, który zareaguje na zmianę użytkownika (currentUserId)
            // Zapewni to załadowanie danych po zalogowaniu.
            LaunchedEffect(currentUserId) {
                if (currentUserId != null) {
                    // Ustaw bieżącego użytkownika w ViewModelu
                    viewModel.setCurrentUser(currentUserId)
                    // Załaduj piosenki (metoda ta sama sprawdzi, czy dodać dane przykładowe)
                    viewModel.loadSongs()
                }
            }
                    ...
                    confirmButton = {
                        Button(
                            onClick = {
                                ...
                                // Sprawdź, czy pola nie są puste ORAZ czy użytkownik jest zalogowany
                                if (dialogTitle.isNotBlank() && dialogArtist.isNotBlank() && currentUserId != null) {
                                    val tempo = dialogTempo.toIntOrNull()?.coerceIn(40, 220) ?: 120

                                    // Stwórz obiekt Song z wymaganym parametrem userId
                                    val newSong = Song(
                                        userId = currentUserId, // <<< POPRAWKA
...
@Composable
// 1. Zmieniona sygnatura funkcji, aby akceptować onSignOut
fun MetronomeApp(onSignOut: () -> Unit) {
    val auth = Firebase.auth
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        // 2. Dodany panel użytkownika z przyciskiem wylogowania
        if (user != null) {
            Row(
                modifier = Modifier
                    .fillMaxWidth()
                ...
                Text(
                    text = user.email ?: "Zalogowano",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant
                )
                // Przycisk wywołuje przekazaną funkcję onSignOut
        ...
        Box(modifier = Modifier.weight(1f)) { // Użyj weight(1f), aby Box zajął pozostałą przestrzeń
            when (currentScreen) {
